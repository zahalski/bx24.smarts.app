...
$eventManager->addEventHandlerCompatible('main', 'OnEndBufferContent', array('CompanyList', 'OnEndBufferContent'), false, 999);

class CompanyList extends IList implements IParams {
    ...
    public function __construct($params, $publicMode=false){
        ...
        if(!empty($params['SMART_FIELDS'])){
            \Awz\Admin\CompanyTable::$fields = $params['SMART_FIELDS'];
        }
...
if(!$checkAuth){
    ...
    if($loadParamsEntity->isSuccess()){
        ...
        $arParams['GRID_OPTIONS']['method_list'] = 'crm.company.list';
        $arParams['GRID_OPTIONS']['method_delete'] = 'crm.company.delete';
        $arParams['GRID_OPTIONS']['method_update'] = 'crm.company.update';
        $arParams['GRID_OPTIONS']['method_add'] = 'crm.company.add';
    ...
    if($arParams['SMART_ID'] && $loadParamsEntity->isSuccess()){
        ...
        if($loadParamsEntity->isSuccess()){
            ...
            if(!empty($arParams['EXT_PARAMS'])){
                $bxRowsResFields = $app->postMethod($arParams['EXT_PARAMS'][1].'crm.company.fields');
            }else{
                $bxRowsResFields = $app->postMethod('crm.company.fields');
            }
    ...
    if($arParams['SMART_ID'] && !$customPrint && $loadParamsEntity->isSuccess()){
        ...
        $fields = \Awz\Admin\CompanyTable::getMap();
        //echo'<pre>';print_r($allFields);echo'</pre>';
